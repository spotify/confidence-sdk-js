...
export function validateKey(key: string): string | null {
  ...
  if (key.length > MAX_KEY_LENGTH) return `key exceeds ${MAX_KEY_LENGTH} characters`;
...
export function validateTagValue(value: string | undefined): string | null {
  ...
  if (value.length > MAX_TAG_VALUE_LENGTH) return `tag value exceeds ${MAX_TAG_VALUE_LENGTH} characters`;
  return null;
}
