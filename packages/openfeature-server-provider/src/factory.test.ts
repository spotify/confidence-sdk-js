import { ConfidenceProviderFactoryOptions, createConfidenceServerProvider } from './factory';
...
/** Evaluates one flag, and reports where the resolve went */
async function resolveOnce(options: Partial<ConfidenceProviderFactoryOptions> = {}) {
  ...
  await provider.resolveStringEvaluation('tutorial-feature', 'default', {}, console);
  const [url, init] = fetchImpl.mock.calls[0]!;
  return { url: String(url), body: JSON.parse(init.body) };
}

describe('createConfidenceServerProvider', () => {
  it('identifies itself to the resolver, so provider traffic can be told apart', async () => {
    const { body } = await resolveOnce();
    expect(body.sdk).toEqual({ id: 'SDK_ID_JS_SERVER_PROVIDER', version: expect.any(String) });
  });
